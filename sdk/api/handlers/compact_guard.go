	...
	go func() {
		...
		sendTimeout := func() {
			if err := CompactTimeoutErrorIfDeadline(state); err != nil {
				// A fired compact deadline wins over any payload buffered for
				// downstream; discard that stale queue before publishing 504.
				for {
					select {
					case <-out:
					default:
		...
		for {
			if CompactTimeoutErrorIfDeadline(state) != nil {
				sendTimeout()
				return
			}
			select {
