...
func TestCompactTimeoutWinsOverQueuedPayload(t *testing.T) {
	ctx, cancel, state := WithCompactAbsoluteTimeout(context.Background(), time.Minute)
	defer cancel()
	remaining := make(chan coreexecutor.StreamChunk, 1)
	remaining <- coreexecutor.StreamChunk{Payload: []byte("late")}
	close(remaining)
	state.markFired()
	result := releaseCompactAbsoluteTimeout(&coreexecutor.StreamResult{Chunks: remaining}, cancel, ctx, context.Background(), state)
	chunk, ok := <-result.Chunks
	if !ok || chunk.Err == nil {
		t.Fatalf("first chunk = %#v, ok=%v; want compact timeout", chunk, ok)
	}
	if got := statusCodeFromCompactTestError(chunk.Err); got != http.StatusGatewayTimeout {
...
func statusCodeFromCompactTestError(err error) int {
